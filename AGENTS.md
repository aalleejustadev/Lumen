...
- `components/ui/` — shadcn/ui primitives. Generated by the CLI; add with `npx shadcn@latest add <name>` rather than hand-writing them, and avoid gratuitous edits since the CLI can overwrite them. Don't run Prettier across this directory — it creates churn in generated files.
  ...
  - **Local fix in `select.tsx`: the popup may be wider than its trigger, and
    an item may ellipsis.** Three lines, all one bug seen from three sides. The
    generated `SelectContent` pins the popup to `w-(--anchor-width)` — exactly
    the trigger's width — over `overflow-x-hidden`, so any option longer than
    the trigger was **hard-clipped mid-word with no ellipsis**. That is fine
    for a select over a short fixed enum, which is what the primitive was drawn
    against, and wrong for every select in this app whose options are *data*:
    the instructor course filters showed "Writing Technical Documenta" and
    "Typography for Product Team" cut off at the edge. It is now
    `min-w-[max(var(--anchor-width),9rem)]` with `max-w-(--available-width)`,
    i.e. the trigger's width is a **floor** and the popup grows to its content
    up to what the viewport allows — the two old floors are combined into one
    declaration because two `min-w-*` classes are the same tailwind-merge group
    and the loser would simply vanish. Alongside it, `SelectItem` gained
    `pr-8`, because the check indicator is `absolute right-2` and an item with
    8px of right padding ran its text underneath the tick; and
    `SelectPrimitive.ItemText` lost `shrink-0` for `min-w-0 … truncate`, since
    `flex-1 shrink-0` is precisely what stops a flex child ever ellipsing. This
    was never Students-specific — the Coupons and Q&A course filters clipped
    identically, and `browse-courses.tsx` had been carrying a partial
    workaround (`min-w-44`) that only helped while the trigger was narrower
    than 176px. `npx shadcn@latest add select` will revert all three; re-apply
    from this note.
...
- `lib/relative-time.ts` — the shared `compactAge` / `compactAgo`. Messages,
  ...
  hour-old row says.
- `components/dashboard/instructor/students/` — **Students** at
  `/dashboard/instructor/students`, from
  `ui-design/light/dashboard/instructor/students-page.png`:
  `students-board.tsx` (client — the two header buttons, the tabs, the course
  filter, the search, the table, the pager and the dialog's open state, since
  one busy descriptor belongs to all of them), `student-row.tsx`,
  `message-all-dialog.tsx`, `students-stats.tsx` (a **Server Component passed
  in as `children`**, so four cards and their icons stay off the bundle —
  `coupons-page.tsx`' arrangement), `students-format.ts`, composed by
  `students-page.tsx`. `lib/instructor-students.ts` reads,
  `lib/config/instructor-students.ts` is every word the page says, and
  `lib/actions/instructor-students.ts` is the three writes. **Nothing on it is
  demo data and it needed no migration** — `Enrollment` already carried every
  column the export asks for, docstrings and all.
  Measured off that export at DPR 2: the instructor shell's usual 32px page
  inset over a full-width column, a header block **pixel-identical** to
  `coupons-page__main.png`'s (so it is that page's, reused rather than
  re-measured) carrying **two** right-aligned buttons instead of one, a four-up
  `CountCard` row of 80px tiles on a 16px gap, a **42px** segmented track
  (`--track`, `p-1`, 34px items) opposite a 42px course filter and a **200 x
  42** search field, then a zero-padding card whose 43px header sits over
  **69px** rows on `--border-subtle` hairlines with cells inset 20px — a 40px
  avatar 10px from a 15px/600 name over a 13px address, and a **225px**
  progress track 7px thick, 14px from its percentage, with the lesson counter
  on a second line beneath it.
  Eight things decide what the page means, and the export settles none of them:
  - **A row is an enrolment, not a person.** The Course column says so, and a
    learner in two of your courses has two different progress bars, so they
    appear twice — the reading My Courses' Students tile already takes.
  - **The headline is the count of the column beneath it.** "Total students"
    ...
    Threads tile records. They are one definition read from two places (the
    counter is a cache of that table) and agree on a real database; they drift
    only in the seeded one, which writes the counter with history the enrolment
    rows do not reproduce. A headline of 1,500 above a list of 14 would be the
    incoherent half of that trade.
  - **"Active this week" is `Enrollment.lastAccessedAt`, not `Session`.** The
    admin Users page counts sessions because a platform operator has no better
    liveness signal; an instructor does, and it is the column the row's own
    Last active already draws — reading it twice is what stops the tile and the
    list telling different stories about one learner.
  - **Completion rate and Avg. rating are the manage page's and My Courses'
    own definitions**, unchanged: `completedAt` over enrolments, and a mean
    weighted by `reviewsCount`. Two instructor screens under one label must not
    show two numbers. Both render an em dash rather than a 0 before there is
    anything to average.
  - **The Quiz column is quizzes *passed* over quizzes in the course, counted
    over distinct quizzes.** `Quiz.retakesAllowed` means one quiz holds several
    attempts, so counting passed *attempts* would let somebody who retook one
    quiz four times read 4/5. It is an **em dash until they have submitted
    something** — what the export draws on its rows at 12% and 8% — and a
    course with no quizzes lands there too, because in both cases a "0/5" is a
    score nobody has taken.
  - **The bar and the counter beside it are allowed to disagree.**
    `Enrollment.progressPercent` is stored and time-weighted, which is the
    whole reason that column exists; deriving the bar from the lesson ratio
    would be the wrong half of the trade My Learning already documents.
  - **Active means "has not finished", not "was here recently."** The export
    puts an Active pill on a row last seen a month ago, so the pill and the
    tabs are one split — which is why the tile above is worded "Active this
    week" rather than reusing the bare word.
  - **The tabs, the course filter, the search and the page live in the URL;
    nothing else does** — all four change which rows exist, which happens in
    SQL. `parseStudentsQuery` is the gate, and a hand-edited `?course=` can
    only narrow, since the query is already scoped by `Course.instructorId`.
- **The export's two header buttons are both real.** A page's primary action
  that opens nothing is the 404 this codebase refuses, and the lead already
  promises the second one ("reach out when someone stalls"):
  - **Export CSV exports what is on screen**, not the whole table — the filter
    row exists to narrow a list, the call `admin-audit.ts` already made. It
    writes the percentage as an integer and the status as a word rather than
    the table's "3 weeks ago", which says nothing in a file that outlives the
    moment it was saved, and it caps at `STUDENTS_EXPORT_LIMIT` with the toast
    saying when the cap bit. Its cells are defused against **CSV injection**: a
    value starting `=`, `+`, `-` or `@` is run as a formula by Excel and
    Sheets, and these cells hold names a stranger typed into a sign-up form.
  - **Message all asks first, and names the real number it will reach.** That
    number is the *current filter's*, which is what makes the control useful
    and the cap acceptable — narrowing the filter is how you aim it.
    `MESSAGE_ALL_LIMIT` is what makes it safe to offer at all on a page whose
    headline says 1,500: each recipient is a `Conversation` plus a `Message`,
    and this is a synchronous write. It reuses an existing thread rather than
    duplicating one, reads its recipients **server-side from the filter** so
    the page cannot be talked into messaging somebody who is not enrolled, and
    is deliberately **not** a transaction — a send that fails on recipient 180
    should leave the first 179 delivered and report what landed rather than
    rolling back messages people have already been notified about.
  - **The row's mail button hands over to `startConversation`** rather than
    opening a thread itself, so there is one answer to "does this conversation
    already exist". It takes the **enrolment** id and resolves the student and
    the course from it under this instructor's own courses — the ownership
    check, rather than two ids from the browser. `messageAllStudents` is the
    one place that duplicates the create, and only because it reads every
    existing thread in one query first rather than one per recipient.
- **`longAgoPrecise` is a third formatter in `lib/relative-time.ts`**, not a
  flag on `longAgo`. That one flattens anything under a day to "Today", which
  is how somebody reads a review; this column is what an instructor scans to
  decide whether a learner has stalled, so the difference between five hours
  and five days has to be on screen. Flattening it would collapse the most
  useful half of the range.
- **The Students row in `instructorNav` is `built: true`**, which lights up
  three things at once: the sidebar row, the manage page's Students row (now
  in `manageRowHref`, filtered to that course through the `?course=` those
...
**`components/dashboard/instructor/course-filter.tsx` is the one "All courses"
filter.** Students, Coupons and Q&A each had the same twenty lines of `Select`
— the same grid icon, the same render-function `SelectValue`, the same "all"
sentinel, down to the same two comments — differing only in a control height
and a copy key. It is one component now for the reason `count-card.tsx` and
`sidebar-nav.tsx` are: a change to how it behaves has to land on all three at
once, which a shared component makes unavoidable and three copies make a matter
of memory. Two decisions live in it rather than in the primitive: it **drops
below the trigger** (`alignItemWithTrigger={false}`, `align="start"`) instead
of Base UI's default of aligning the selected item *over* the trigger, which
with six courses covered the trigger and the search field beside it and moved
depending on which course was selected — the call `browse-courses.tsx` already
made for its own filters, and it restores the slide-in that
`data-[align-trigger=true]:animate-none` suppresses; and the **trigger**
truncates while the popup does not, since the menu is the only place the choice
is actually made. That truncation is a span plus
`**:data-[slot=select-value]:min-w-0`, **not** the `line-clamp-1`
`SelectTrigger` already applies — that class is dead on this element, because
the same rule sets the value to `display: flex` and `-webkit-line-clamp` needs
`-webkit-box`. Worth knowing before reaching for `line-clamp-*` on any
`SelectValue`.
