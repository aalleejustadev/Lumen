...
// Heights are measured off the export (all bars share a 502px baseline);
// every bar is 22px wide on a 4px radius, and today's is a violet-to-blue
// gradient rather than a flat fill.
const activity = [
  { day: "M", height: "h-[26px]" },
  { day: "T", height: "h-[39px]" },
  { day: "W", height: "h-[21px]" },
  { day: "T", height: "h-[39px]", today: true },
  { day: "F", height: "h-[33px]" },
  { day: "S", height: "h-[17px]" },
  { day: "S", height: "h-[30px]" },
]
...
function WeeklyGoalDonut() {
  ...
  return (
    <div className="relative grid place-items-center">
      <svg viewBox="0 0 130 130" className="size-26 -rotate-90">
        <circle
          cx="65"
          cy="65"
          r={radius}
      ...
      <div className="absolute grid place-items-center">
        <span className="stat-figure text-xl">72%</span>
...
function HeroAppPreview({ className }: { className?: string }) {
  return (
    ...
    >
      {/* browser chrome */}
      <div className="flex h-11 items-center gap-4 border-b bg-soft px-5">
        <div className="flex gap-2">
          <span className="size-3 rounded-full bg-destructive" />
          <span className="size-3 rounded-full bg-star" />
          <span className="size-3 rounded-full bg-success" />
        </div>
        <div className="mx-auto flex h-7 items-center gap-2 rounded-lg bg-card px-3 text-xs text-muted-foreground">
      ...
      <div className="flex">
        {/* sidebar */}
        <aside className="hidden w-[207px] shrink-0 flex-col gap-3.5 border-r bg-sidebar p-3.5 md:flex">
        ...
        {/* canvas */}
        <div className="flex min-w-0 flex-1 flex-col gap-3 bg-background p-5">
          ...
          <div className="grid gap-3 lg:grid-cols-[1.354fr_1fr]">
            <Card className="[--card-spacing:--spacing(3)]">
              <CardHeader>
                <CardTitle className="text-[13px]">Continue learning</CardTitle>
            ...
            <Card className="[--card-spacing:--spacing(3)]">
              <CardHeader>
                <CardTitle className="text-[13px]">Weekly goal</CardTitle>
              </CardHeader>
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            {stats.map((stat) => (
              <Card key={stat.label} className="[--card-spacing:--spacing(2)]">
                <CardHeader>
                ...
                <CardContent className="leading-none">
                  <span className="stat-figure text-xl">{stat.value}</span>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <Card className="[--card-spacing:--spacing(2)]">
            <CardHeader>
              <CardTitle className="text-[13px]">Activity this week</CardTitle>
            ...
            <CardContent className="flex items-end justify-between gap-2">
              {activity.map((entry, index) => (
                ...
                >
                  <span
                    className={cn(
                      "w-full max-w-[22px] rounded-[4px] bg-track",
                      entry.height,
                      entry.today &&
                        "bg-gradient-to-b from-accent-1 to-accent-2"
                    )}
                  />
                  <span className="text-[10px] leading-none text-subtle-foreground">
                    {entry.day}
                  </span>
                </div>
              ))}
